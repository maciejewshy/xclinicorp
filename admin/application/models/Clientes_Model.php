...
class Clientes_Model extends CI_Model
{
  ...
  public function getClientes($id = null)
  {
    if (isset($id['id_usuario'])) {
      ...
      if ($Id != 1) {
        ...
        $this->db->where('usuarios_tb.automatico', 1);
        $this->db->join('usuarios_tb', 'usuarios_tb.id_usuario = clientes_tb.id_usuario', 'left');
        if (isset($id['id_cliente'])) {
          $this->db->where('clientes_tb.id_cliente', $id['id_cliente']);
        }
        $dados = $this->db->get('clientes_tb');
      } else {
        $dados = $this->db->get('clientes_tb');
      }
    } elseif (isset($id['id_cliente'])) {
      $ID['id_cliente'] = $id['id_cliente'];
      $dados = $this->db->get_where('clientes_tb', $ID);
    } else {
      $dados = $this->db->get('clientes_tb');
    }
  ...
  {
    $ids = $id['id_cliente'];
    if (isset($id['id_cliente']) && !empty($id['id_cliente'])) {
      ...
      $this->db->where('mensagens_tb.id_cliente', $ids);
  ...
  public function getMensagem($dados, $id_cliente)
  {
    $this->db->select('*');
    $this->db->where('tiposcanal_tb.tipo', $dados);
    $this->db->where('canais_tb.id_cliente', $id_cliente);
    $this->db->join('canais_tb ', 'mensagens_tb.id_canal = canais_tb.id_canal');
  ...
  public function updateClienteData($dados)
  {
      $this->db->where('id_cliente', $dados['id_cliente']);
      return $this->db->update('clientes_tb', $dados);
  }
}
